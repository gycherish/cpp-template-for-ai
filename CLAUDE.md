...
- **先对齐再动手**: 用户以疑问句或明显商量语气提出的内容是待讨论的方案, 应先给出分析并与用户核对结论, 达成一致后再动手修改; 不得把疑问直接当指令执行
...
本仓库需要保存后续人工与 AI 继续正确开发所需的事实, 不保存一次性推理过程。

应纳入版本控制的正式资料:

- `doc/product/`: 需求、PRD、用户故事、验收标准与产品边界。
- `doc/design/`: 架构设计、模块设计、接口设计、数据模型与方案权衡。
- `doc/adr/`: 重要且长期有效的技术/产品决策记录。
- `doc/dev/`: 开发流程、工程规范、工具链、AI 协作规则, 以及各子系统的内部设计与实现细节。

不应纳入版本控制的临时资料:

- AI 生成的长篇分析草稿、头脑风暴、未确认方案。
- 一次性截图、调试记录、临时对话摘录。
- 含密钥、客户资料、内部链接或其他敏感信息的材料。
- 体积很大的二进制设计源文件; 如必须版本化, 应先评估 Git LFS。

临时资料统一放入当前 AI 工具工作目录(如 `.claude/`、`.codex/`), 经人工确认后再整理为 `doc/` 下的正式文档。

撰写 `doc/` 下正式文档时:

- Markdown 每个段落单独成行, 不手动换行(编辑器与渲染器会自动折行)。
- 导航页(各 `index`)从读者视角撰写, 假设所引用的文档已经存在, 不写"待补充""新增时登记"之类编写者备注。
- 架构/设计类文档(`doc/design/`、`doc/adr/`)只写稳定的结构与契约(分层、模块职责、语义约定、数据模型概念), 不包含函数签名、目录树等易随代码漂移的细节; 代码细节以头文件与生成的 API 文档为准。
